...
const i18n = {
  en: {
    ...
    // about.html
    aboutTitle: "About",
    aboutBrief: "This site is a long-term personal structure that gathers work, ideas, systems, and research directions that continue to evolve. The material exists in different states: some pieces are relatively complete, while others are still being revised, reorganized, or expanded. It functions more like an ongoing workspace than a fixed presentation. What matters here is not only the outcome, but also how things are structured, clarified through revision, and allowed to develop over time.",
    aboutProfileTitle: "Profile",
    aboutProfile: "My work moves across several intersecting directions, including research, programming, design, writing, and experimental creation. These areas are not treated as separate domains, but as different ways of engaging with similar questions. Some work takes the form of text, some becomes systems or tools, and some appears as visual or auditory expression. Beyond the medium itself, I am primarily concerned with how things are constructed and how clarity can be maintained within complexity.",
    aboutFocusTitle: "Focus",
    aboutFocus: "The work here tends to revolve around several recurring directions. One is analytical, involving reading, comparison, and structuring complex material. Another is technical, focusing on tools, systems, and long-term frameworks. A third is more expressive, working through image, sound, and form. These directions are not isolated; they intersect and shift across different projects, forming new combinations over time.",
    aboutApproachTitle: "Approach",
    aboutApproach: "I usually begin with structure and then move into the content itself. Categories, naming, and organization are continuously adjusted along the way. Revision is an ongoing process: some parts are rewritten, some are reorganized, and others are removed. The site itself follows the same logic—it operates as a continuously edited environment rather than a finished endpoint.",
    // archive.html
  ...
  zh: {
    ...
    // about.html
    aboutTitle: "关于",
    aboutBrief: "这个网站是一个长期使用的个人结构，用来整理作品、想法、系统，以及一些仍在延展中的研究线索。这里的内容处在不同的状态之中：有些已经相对完整，有些还在不断被修改、拆解或重组。它更接近一个持续运作的工作空间，而不是一次性完成的展示页面。对我来说，结果固然重要，但同样重要的是内容如何被组织、如何在修改中逐渐清晰，以及它在时间中如何继续生长。",
    aboutProfileTitle: "个人简介",
    aboutProfile: "我的工作分布在几个相互交错的方向之间：研究、编程、设计、写作，以及一些带有实验性的创作。它们并不会被严格区分，更多时候是在不同形式中回应相似的问题。有的内容以文字呈现，有的成为系统或工具，也有的转化为视觉或声音上的表达。相比具体的媒介，我更关注的是这些内容如何被构建，以及在复杂性中保持结构与清晰。",
    aboutFocusTitle: "关注点",
    aboutFocus: "这里的工作大致围绕几个持续出现的方向展开：一部分偏向分析，通过阅读、比较与整理，将复杂内容转化为更清晰的结构；一部分偏向技术，涉及工具、系统以及可以长期使用的框架；还有一部分更接近创作，通过图像、声音或形式进行表达。这些方向并不是孤立存在的，它们会在具体项目中不断交叉、变化，并形成新的组合方式。",
    aboutApproachTitle: "方法",
    aboutApproach: "我通常先建立整体结构，再逐步推进具体内容。分类、命名和组织方式会在过程中不断调整。修改是持续发生的：有些内容会被重写，有些会被拆分或重新组合，也有一些会被删除。这个网站本身也在经历同样的过程，它更像一个不断被编辑的环境，而不是一个已经完成的终点。",
    // archive.html
    archiveTitle: "归档",
